...
SELECT Teams.Name as 'Team', AVG(Age) as 'Average Age'
FROM Players_List
...
--NOT WORKING!! Team whose location has de postal code between 10000 and 99999
SELECT Teams.Name as 'Team', Locations.Name as 'Location', Locations.Postal_Code
FROM Locations
GROUP BY Teams.Name


--Referees who have only one assigned game
SELECT Referees.Name as 'Referee', COUNT (*) as 'No of Games'
FROM Games
JOIN Referees ON Games.Referee_ID=Referees.ID
GROUP BY Referees.Name
HAVING COUNT(*) > 0